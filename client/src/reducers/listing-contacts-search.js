...
export const diffListingContacts = allContacts => {
  const state = store.getState();
  const { listingContacts } = state.listingContacts;
  console.log("DIFF", {
    allContacts,
    listingContacts
  })
...
export const searchListingContacts = query => async dispatch => {
  const state = store.getState();
  const { limit } = state.listingContactsSearch;
  try {
    dispatch(isLoading(true));
    const res = await axios.get(`/api/contacts/?limit=${limit}&offset=${0}&query=${query}`);
    console.log("RESPONSE", res.data);
    const diffedContacts = diffListingContacts(res.data.rows);
...
export const listingContactsSearch = (state = initialState, action) => {
  switch (action.type) {
    case SET_IS_LOADING:
      return {
        ...state,
...
export default listingContactsSearch;
